    ...
    <>
      <section className="py-section-gap px-margin-mobile md:px-margin-desktop">
        ...
        <div className="max-w-container-max-width mx-auto grid grid-cols-1 md:grid-cols-2 gap-element-gap">
          {pets.map((pet, index) => (
            <div key={index} className="group flex flex-col h-full bg-surface-container-low rounded-[2rem] p-6 border border-outline-variant/10 shadow-sm hover:shadow-md transition-all">
              ...
              <div className="flex justify-between items-center mb-4">
                ...
                <span className="px-4 py-1.5 bg-primary/10 rounded-full text-xs font-bold text-primary uppercase tracking-wider">
                  {pet.typeInfo}
                </span>
              </div>
      ...
      {activePet && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 md:p-8 bg-black/60 backdrop-blur-sm transition-opacity">
          ...
          <div className="absolute inset-0" onClick={() => setActivePet(null)}></div>
          
          <div className="bg-white rounded-[2rem] max-w-2xl w-full max-h-[90vh] flex flex-col shadow-2xl overflow-hidden relative z-10 transform transition-all scale-100 opacity-100">
            <button 
            ...
            <div className="h-48 md:h-64 relative shrink-0">
              ...
              <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/30 to-transparent flex items-end p-6 md:p-8">
                <h3 className="text-3xl md:text-4xl font-black text-white">Sobre {activePet.name}</h3>
              </div>
            </div>
            
            <div className="p-6 md:p-8 overflow-y-auto space-y-4 bg-white">
              {activePet.fullDescription.map((paragraph, idx) => (
                <p key={idx} className="text-on-surface-variant leading-relaxed text-sm md:text-base font-medium">
                  {paragraph}
                </p>
              ))}
            </div>
